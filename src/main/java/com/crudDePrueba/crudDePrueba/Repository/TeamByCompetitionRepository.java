...
@Repository
public interface TeamByCompetitionRepository extends JpaRepository<TeamCompetition,Long> {
    @Query(value = "SELECT c.name,c.prize,tc.matches_played FROM competitions c join team_by_competition tc on tc.competition_id = c.id WHERE tc.team_id =:id",nativeQuery = true)
